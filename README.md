...
- **聚合搜索(元搜索)**:本地索引搜不到时,自动借用必应/百度/360搜索/搜狗/谷歌/中国搜索,每引擎翻页抓取 3 页(不只第一页),结果去重缓存到本地引擎,分页展示;**结果自动编入引擎**——抓取页面存入 crawled,重建索引后本地即可搜到(不用再等外部引擎);搜狗等引擎的相对路径链接自动补齐为绝对 URL(不会 404);管理后台可启停各引擎、添加自定义引擎(URL 模板 `{q}` 查询词、`{p}` 翻页);`engine.conf` 可配置 `meta_proxy`(如 `127.0.0.1:7890`)使谷歌等经代理可达
- **加载动画**:搜索等待时显示旋转动画 + 动态提示(本地无结果自动借外部引擎);管理后台爬虫运行时显示状态动画
